...
class Convolution(nn.Sequential):
    ...
    def __init__(
        ...
        max_pool_kernel: Sequence[int] | int = None,
        ...
        if max_pool_kernel is not None:
            if isinstance(max_pool_kernel, int):
                max_pool_kernel = (max_pool_kernel,) * self.spatial_dims
            max_pool = nn.MaxPool3d(
                kernel_size=max_pool_kernel, ceil_mode=True
            )
            self.add_module("maxpool", max_pool)
...
class AxialDWConv(nn.Module):
    ...
    def __init__(
        ...
        self.add_module("conv_d", conv_d)
        self.add_module("conv_h", conv_h)
        self.add_module("conv_w", conv_w)
...
class ADWConvUnit(nn.Module):
    ...
    def __init__(
        ...
        out_channels: int,
        strides: Sequence[int] | int = 1,
        adw_kernel_size: Sequence[int] | int = 7,
        kernel_size: Sequence[int] | int = 3,
        max_pool_kernel: Sequence[int] | int = None,
        subunits: int = 2,
        adn_ordering: str = "NDA",
...
class DAGCUNit(nn.Module):
    ...
    def __init__(
        ...
        for su in range(subunits):
            ...
            if su <= (subunits - 2):
                ...
                unit = Convolution(
                    self.spatial_dims,
                    int(schannels*fraction),
                    int(out_channels-schannels*(1-fraction)),
                    strides=sstrides,
                    kernel_size=kernel_size,
                    adn_ordering=adn_ordering,
                    act=None,
        ...
        if max_pool_kernel is not None:
            max_pool = nn.MaxPool3d(kernel_size=max_pool_kernel, ceil_mode=True)
    ...
    def _cosine_similarity_matrix(self, feature_map: torch.Tensor, threshold: float) -> torch.Tensor:
        ...
        count_smaller = torch.sum(mask, dim=-1)

        return count_smaller, upper_triangular
    
    def _select_top_channels(self, x: torch.Tensor, fraction: float) -> torch.Tensor:
        b, c, d, h, w = x.shape
        num_select = int(c * fraction)

        x_similar, upper = self._cosine_similarity_matrix(x, threshold=self.threshold)
